...
"""
训练神经网络模型并保存训练过程中的日志和模型参数
"""
class Trainer:
    def __init__(self, model, optimizer, loss, dataloader, n_epochs, eval_step):
        self.model = model # 神经网络模型
        self.optimizer = optimizer # 优化器对象，更新模型参数
        self.loss = loss # 损失函数对象，计算模型的损失
        self.dataloader = dataloader # 数据加载器对象，提供训练和验证数据
        self.n_epochs = n_epochs # 训练轮数
        self.eval_step = eval_step # 评估步数，指定每隔多少步评估一次模型性能

        self.train_loss = []
        self.train_acc = []
        self.valid_loss = []
        self.valid_acc = []
        # 训练过程中保存的模型参数，epoch 为 key，模型为 value
        
        self.model_cache = {}
        self.train_log = []
        # 训练日志，记录每个 epoch 的训练和验证损失、准确率等信息
